<?php 
    ...
    $name = "";
    $user = explode(" ", $_SESSION['user']['name']);
    foreach($user as $u){
        $name.= ucfirst($u)[0];
    }
?>
...
<nav class="border-b border-opacity-25 border-sky-300 bg-sky-600"
            ...
            <div class="flex items-center px-2 lg:px-0">
                <div class="hidden sm:block">
                    ...
                    <?php foreach($menus as $menu){ ?>
                        <a href="<?= $menu['url'] ?>"
                            class="px-3 py-2 text-sm font-medium text-white rounded-md <?= $_SERVER['REQUEST_URI'] === $menu['url'] ? ' bg-sky-700' : '' ?>"><?= $menu['title'] ?></a>
                    <?php } ?>
                    ...
                    <div>
                        <button @click="open = !open" type="button"
                            ...
                            <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-sky-100">
                                <span class="font-medium leading-none text-sky-700"><?= $name ?></span>
                            </span>
                            <!-- <img
    ...
    <div x-show="mobileMenuOpen" class="sm:hidden" id="mobile-menu">
        <div class="pt-2 pb-3 space-y-1">
            <?php foreach($menus as $menu){ ?>
                <a href="<?= $menu['url'] ?>"
                class="block px-3 py-2 text-base font-medium text-white rounded-md hover:bg-sky-500 hover:bg-opacity-75"><?= $menu['title'] ?></a>
            <?php } ?>
